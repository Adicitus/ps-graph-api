function New-GraphAPICalendarEvent {
    [CmdletBinding()]
    param(
        ...
        [String]$User,
        [Parameter(Mandatory=$false, HelpMessage="Calendar ID for the calendar to create the event in. Uses the user's default calendar if not specified.")]
        [String]$CalendarID,
        [Parameter(Mandatory=$true, ParameterSetName="Hashtable", HelpMessage="Event object (See https://docs.microsoft.com/en-us/graph/api/resources/event?view=graph-rest-1.0).")]
        [hashtable]$EventBody,
        [Parameter(Mandatory=$true, ParameterSetName="Params", HelpMessage="Subject for the Meeting.")]
        [string]$Subject,
        [Parameter(Mandatory=$true, ParameterSetName="Params", HelpMessage="Meeting start time.")]
        [datetime]$Start,
        [Parameter(Mandatory=$true, ParameterSetName="Params", HelpMessage="Meeting end time.")]
        [datetime]$End,
    ...
    if ($CalendarID) {
        $uri = "https://graph.microsoft.com/v1.0/users/{0}/calendar/{1}/events" -f $encodedUser, $CalendarID
    } else {
        $uri = "https://graph.microsoft.com/v1.0/users/{0}/events" -f $encodedUser
    }
